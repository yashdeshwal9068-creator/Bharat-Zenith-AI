 ...
 *   Mistral       — text via mistral-small-latest;
 ...
 *                   vision via Llama-3.2-11B-Vision-Instruct
 ...
 *               maxOutputTokens is fixed at 8000 so the model
 *               has room to complete long answers within Vercel's
 *               10-second edge function window. A conciseness
 *               system instruction is also prepended to every
 *               request so Gemini finishes within the budget.
 ...
 *   - Fireworks: correct catalog slug is
 *               accounts/fireworks/models/llama-3.1-8b-instruct
 *               (standard dot notation as listed in Fireworks'
 ...
 *   - HuggingFace: model ID from the spec is intact.
 *   - Cohere:   Using command-r-latest (stable production alias,
 *               always points to the current recommended Command R
 *               revision without snapshot lock-in).
 *   - Mistral:  open-mistral-7b is deprecated. Using
 *               mistral-small-latest for text instead.
 *   - GitHub Models: gpt-4o-mini (no vendor prefix) via the
 *               official https://models.github.ai/inference/
 *               chat/completions endpoint, authenticated with a
 *               GitHub PAT / GITHUB_TOKEN that has "models: read"
 *               scope.
 *   - SambaNova: Meta-Llama-3.3-70B-Instruct for text;
 *               Llama-3.2-11B-Vision-Instruct (vision) when an
 *               image is attached, via SambaNova Cloud's
...
async function callGemini(messages, temperature, max_tokens, timeoutMs, attachment) {
  ...
  // Prepend a conciseness instruction so Gemini finishes its full
  // response within Vercel's 10-second edge-function window.
  // Any system text extracted from the conversation is appended
  // after the prefix so it is never discarded.
  const GEMINI_CONCISENESS_PREFIX =
    'Be highly concise, direct, and provide step-by-step solutions ' +
    'without unnecessary introductory or concluding text. ';
  const systemInstruction = systemText.trim()
    ? GEMINI_CONCISENESS_PREFIX + systemText.trim()
    : GEMINI_CONCISENESS_PREFIX.trim();

  const body = {
    contents,
    generationConfig: {
      temperature: temperature !== undefined ? temperature : 0.7,
      maxOutputTokens: 8000,   // fixed — gives the model room to complete long answers
    },
...
/* ════════════════════════════════════════════════════
   ...
   ─────────────────────────────────────────────────
   Text:   mistral-small-latest (open-mistral-7b is deprecated)
...
async function callMistral(messages, temperature, max_tokens, timeoutMs, attachment) {
  ...
  // mistral-small-latest — open-mistral-7b is deprecated; this is the current free-tier text model
  // pixtral-12b-2409     — free tier, Mistral's dedicated vision model
  const model = isVision ? 'pixtral-12b-2409' : 'mistral-small-latest';
...
const PROVIDERS = {
  ...
  githubmodels: {
    ...
    call: (messages, temperature, max_tokens, timeoutMs, attachment) => callOpenAICompatible({
      ...
      model: 'gpt-4o-mini',
      visionModel: 'gpt-4o-mini',        // same model handles text + vision natively
      supportsVision: true,
  ...
  mistral: {
    name: 'Mistral',
    vision: true,  // text: mistral-small-latest; images: pixtral-12b-2409
  ...
  sambanova: {
    name: 'SambaNova',
    vision: true, // text: Meta-Llama-3.3-70B-Instruct; images: Llama-3.2-11B-Vision-Instruct
    call: (messages, temperature, max_tokens, timeoutMs, attachment) => callOpenAICompatible({
