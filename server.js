 ...
 * ─────────────────────────────────────────────────────
 * 8-ENGINE ARCHITECTURE WITH AUTOMATIC FAILSAFE FALLBACK
 *
 ...
 *   Mistral   — vision via pixtral-12b-2409 model;
 *               text-only falls back to mistral-small-latest
 * All other engines are text-only. If a non-vision engine
 * is selected with an image attached, the backend
 ...
 * All 8 are still small/fast/free-tier-safe as requested.
 * ───────────────────────────────────────────────────────
 *
...
/* ════════════════════════════════════════════════════
   ...
   ─────────────────────────────────────────────────
   Text-only requests:  mistral-small-latest (free tier)
   Vision requests:     pixtral-12b-2409 (free tier,
                        Mistral's dedicated multimodal model)

   Mistral uses the standard OpenAI-compatible
   /v1/chat/completions endpoint for both modes.
   For vision, message content is sent as an array
   with a text part and an image_url part (base64 URL),
   matching the official Pixtral API schema.
════════════════════════════════════════════════════ */
async function callMistral(messages, temperature, max_tokens, timeoutMs, attachment) {
  const apiKey = process.env.MISTRAL_API_KEY;
  if (!apiKey) throw new Error('Mistral: API key not set on the server (check Vercel env vars).');
  ...
  // ── TEXT-ONLY: delegate to the shared OpenAI-compatible caller ────
  ...
  // ── VISION: build multimodal content for the last user turn ───────
  // Pixtral expects:
  //   { role: 'user', content: [
  //       { type: 'text',      text: '...' },
  ...
  for (let i = 0; i < flat.length; i++) {
    const m = flat[i];
    if (i === flat.length - 1 && m.role === 'user') {
      // Final user turn: inject the image alongside the text prompt
      outgoingMessages.push({
        role: 'user',
        content: [
          { type: 'text',      text: m.content || 'Describe what you see in this image.' },
          { type: 'image_url', image_url: { url: `data:${attachment.mimeType};base64,${cleanData}` } },
        ],
      });
    } else {
  ...
  try {
    ...
    if (err?.name === 'AbortError') throw new Error(`Mistral: timed out after ${Math.round((timeoutMs || PROVIDER_TIMEOUT_MS) / 1000)}s.`);
  ...
  const data = await res.json();
  const content = data?.choices?.[0]?.message?.content;
  if (!content || !String(content).trim()) throw new Error('Mistral: returned an empty response.');
...
const PROVIDERS = {
  ...
  mistral: {
    name: 'Mistral',
    vision: true,  // text via mistral-small-latest; images via pixtral-12b-2409
...
/* Order used to pick automatic fallbacks. Fastest / most reliable
   ...
   free inference is the most rate-limited of the eight. */
const FALLBACK_ORDER = ['gemini', 'groq', 'cohere', 'openrouter', 'mistral', 'fireworks', 'nvidia', 'huggingface'];
