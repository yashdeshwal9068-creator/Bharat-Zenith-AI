...
/* ══════════════════════════════════════════
   ...
   10 engines · 3 free-tier providers only:
   Gemini ×5, OpenRouter ×4, Mistral ×1.
   Order = "Intelligence Wise" ranking.
   textOnly:true → engine cannot see images; the
   4 OpenRouter engines get disabled in the dropdown
   while an image is attached (see IMAGE-AWARE ENGINE
   LOCKING below).
══════════════════════════════════════════ */
const ENGINES = [
  { key:'gemini',       name:'Gemini 2.5 Flash',     sub:'Google · gemini-2.5-flash',                    color:'#4285F4', textOnly:false },
  { key:'geminipro',    name:'Gemini 2.5 Pro',       sub:'Google · gemini-2.5-pro · vision',             color:'#3367D6', textOnly:false },
  { key:'geminilite',   name:'Gemini 2.5 Flash-Lite',sub:'Google · gemini-2.5-flash-lite',                color:'#669DF6', textOnly:false },
  { key:'gemini3',      name:'Gemini 3 Flash',       sub:'Google · gemini-3-flash-preview',              color:'#1A73E8', textOnly:false },
  { key:'gemini31lite', name:'Gemini 3.1 Flash-Lite',sub:'Google · gemini-3.1-flash-lite',                color:'#8AB4F8', textOnly:false },
  { key:'orreasoning',  name:'DeepSeek V4 Flash',    sub:'OpenRouter · deepseek-v4-flash:free',          color:'#8b5cf6', textOnly:true  },
  { key:'or70b',        name:'Llama 3.3 70B',        sub:'OpenRouter · llama-3.3-70b-instruct:free',     color:'#a78bfa', textOnly:true  },
  { key:'orcoder',      name:'Qwen3 Coder',          sub:'OpenRouter · qwen3-coder:free',                color:'#7c3aed', textOnly:true  },
  { key:'orfast',       name:'Gemma 4 26B',          sub:'OpenRouter · gemma-4-26b-a4b-it:free',         color:'#c4b5fd', textOnly:true  },
  { key:'mistral',      name:'Mistral Small',        sub:'Mistral · mistral-small-latest · Pixtral Vision', color:'#FF7000', textOnly:false },
];
const engineByKey = k => ENGINES.find(e => e.key === k) || ENGINES.find(e => e.key === 'gemini') || ENGINES[0];

let selectedEngine = localStorage.getItem('bz_engine') || 'gemini';
...
/* ══════════════════════════════════════════
   ...
   ─────────────────────────────────────────
   The 4 OpenRouter free-tier engines (DeepSeek V4
   Flash, Llama 3.3 70B, Qwen3 Coder, Gemma 4 26B)
   are treated as text-only here. As soon as an image
   attachment is pending we disable them in the
   dropdown. If one of them was already the active
   selection, we auto-switch to Gemini so the request
   actually reaches a vision-capable engine. Clearing/
   removing the image re-enables all four.
══════════════════════════════════════════ */
...
function renderBharatAgentSummary(subtasks){
  ...
  <div class="bsc-header" onclick="toggleAccordion('${id}')">
    <span class="bsc-icon">🧬</span>
    <span class="bsc-label">Bharat Agent: ${subtasks.length} sub-tasks ›</span>
    <span class="bsc-chevron">›</span>
  </div>
